...
def random_crop(x, center_crop_size):

    w, h = x.shape[0] , x.shape[1]
    sizew, sizeh = center_crop_size[0], center_crop_size[1]
    random_w = w - sizew + 1
    random_h = h - sizeh + 1
    random_w = np.random.randint(random_w)
    random_h = np.random.randint(random_h)
    cropped = x[random_w: random_w + sizew,
                random_h: random_h + sizeh, :]

    return cropped

# 读取图片,并得到固定大小
def scale_byRatio(img_path, ratio=1.0, return_width=299, return_hight=299, crop_method=center_crop):
    ...
    img_resized = cv2.resize(img_cropped, (return_width, return_hight), interpolation=cv2.INTER_CUBIC)
    img_rgb = img_resized
...
seq = iaa.Sequential(
    [
        ...
        sometimes(iaa.Affine(
            scale={"x": (0.8, 1.20), "y": (0.8, 1.20)}, # scale images to 80-120% of their size, individually per axis
...
def generator_batch(data_list, nbr_classes=3, batch_size=32,batch_index = 0,return_label=True,
    ...
    while True:
        ...
        if N >= (current_index + batch_size):
            ...
            current_index = current_index - batch_size + current_batch_size
            current_batch_size = batch_size
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            img = scale_byRatio(img_path, ratio=scale_ratio, return_width=img_width, return_hight=img_height,
